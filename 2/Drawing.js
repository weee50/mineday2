const LOCAL_ASSETS = ""
const GLOBAL_ASSETS = "../assets/"
const COLOR_ASSETS = "../color-assets/"

var prefix = GLOBAL_ASSETS;
...
var images = {
    ...
    special:
    {
        unopened: imageFromSrc(prefix + "Unopened.png"),
        noMines: imageFromSrc(prefix + "Ground_Blank.png")
    }
};
...
numbersNeeded = range(-8, 25)

const NEED_TO_LOAD = {
    mines: [1, 3, 5], // also controls basically everything except numbers
...
for (i = 0; i < NEED_TO_LOAD.mines.length; i++)
{
    ...
    images.mines[mineToLoad] = imageFromSrc(prefix + "Mine_" + mineToLoad + ".png");
    images.flags[mineToLoad] = imageFromSrc(prefix + "Flag_" + mineToLoad + ".png");
    images.flags_won[mineToLoad] = imageFromSrc(prefix + "Flag_Won_" + mineToLoad + ".png");
    images.opened[mineToLoad] = imageFromSrc(prefix + "Mine_" + mineToLoad + ".png");
    images.clicked[mineToLoad] = imageFromSrc(prefix + "Mine_Open_" + mineToLoad + ".png");
}

for (i = 0; i < NEED_TO_LOAD.numbers.length; i++)
{
    numToLoad = NEED_TO_LOAD.numbers[i];
    images.numbers[numToLoad] = imageFromSrc(prefix + "Ground_" + numToLoad + ".png");
}
...
function drawBoard()
{
  ...
  // Updates counters
  let mineListHTML = "<img src=\"" + prefix + "Unopened.png\">: " + (boardGenerated ? boardData.blankSquares - unopenedCount : 0) + "/" + boardData.blankSquares + "<br>";
  for (let i = 0; i < boardData.mines.length; i++)
  {
    currentMine = boardData.mines[i];
    mineListHTML += "<img src=\"" + prefix + "Flag_" + currentMine.mine.texture_suffix + ".png\">: " +
